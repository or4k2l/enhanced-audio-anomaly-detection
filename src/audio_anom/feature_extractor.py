...
class RobustFeatureExtractor:
    ...
    def _handle_nans(self, features: np.ndarray, feature_name: str = "features") -> np.ndarray:
        ...
        if np.isnan(features).any():
            ...
            logger.warning(f"Found {n_nans} NaN values in {feature_name}, replacing with zeros")
    ...
    def extract_mel_spectrogram(self, audio: np.ndarray) -> Optional[np.ndarray]:
        ...
        try:
            if len(audio) < self.n_fft:
                logger.warning(f"Audio too short ({len(audio)} samples) for mel-spectrogram")
                return None
    ...
    def extract_spectral_features(self, audio: np.ndarray) -> Optional[Dict[str, np.ndarray]]:
        ...
        try:
            if len(audio) < self.hop_length:
                logger.warning(f"Audio too short ({len(audio)} samples) for spectral features")
                return None
    ...
    def extract_temporal_features(self, audio: np.ndarray) -> Optional[Dict[str, float]]:
    ...
    def extract_features(self, audio: np.ndarray, target_sr: Optional[int] = None) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if target_sr is not None and target_sr != self.sr:
                try:
                    audio = librosa.resample(audio, orig_sr=target_sr, target_sr=self.sr)
                except Exception as e:
            ...
            if len(feature_vector) < target_dim:
                ...
                logger.debug(f"Padded feature vector from {len(feature_vector)-padding} to {target_dim}")
    ...
    def extract_features_batch(
        ...
        for i, audio in enumerate(audio_list):
            ...
            if feat is not None:
                ...
                logger.warning(f"Failed to extract features for audio {i}, using zero vector")
