...
class KNNDetector(BaseEmbeddingDetector):
    ...
    def fit(self, X: np.ndarray) -> "KNNDetector":
        ...
        """
        logger.info(
            f"Fitting KNNDetector on {X.shape[0]} samples with k={self.n_neighbors}"
        )
...
class EnsembleDetector(BaseEmbeddingDetector):
    ...
    def fit(self, X: np.ndarray) -> "EnsembleDetector":
        ...
        logger.info(
            f"EnsembleDetector fitted, ensemble threshold: {self.threshold:.4f}"
        )
...
def create_detector(method: str, **kwargs) -> BaseEmbeddingDetector:
    ...
    if method == "mahalanobis":
        ...
        raise ValueError(
            f"Unknown method: {method}. Choose from: mahalanobis, knn, isolation_forest, ensemble"
        )
