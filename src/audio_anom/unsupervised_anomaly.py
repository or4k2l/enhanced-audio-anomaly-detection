...
class IsolationForestAnomalyDetector(BaseUnsupervisedAnomalyDetector):
    ...
    def fit(self, X: np.ndarray) -> "IsolationForestAnomalyDetector":
        ...
        Returns:
            self: Fitted estimator
        """
        logger.info(f"Fitting Isolation Forest with n_estimators={self.n_estimators}, "
                   f"contamination={self.contamination}")
        ...
        logger.info(f"Isolation Forest fitted on {X.shape[0]} samples with {X.shape[1]} features")
        return self
...
class EllipticEnvelopeAnomalyDetector(BaseUnsupervisedAnomalyDetector):
    ...
    def fit(self, X: np.ndarray) -> "EllipticEnvelopeAnomalyDetector":
        ...
        Returns:
            self: Fitted estimator
        """
        logger.info(f"Fitting Elliptic Envelope with contamination={self.contamination}")
        ...
        logger.info(f"Elliptic Envelope fitted on {X.shape[0]} samples with {X.shape[1]} features")
        return self


def create_detector(method: str, contamination: float = 0.1, **kwargs) -> BaseUnsupervisedAnomalyDetector:
    ...
    if method == 'lof':
        ...
        raise ValueError(f"Unknown method: {method}. Choose from 'lof', 'isolation_forest', 'elliptic_envelope'")
