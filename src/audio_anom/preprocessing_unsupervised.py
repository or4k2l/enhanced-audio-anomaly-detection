...
class UnsupervisedPreprocessor:
    ...
    def fit(self, X: np.ndarray) -> "UnsupervisedPreprocessor":
        ...
        """
        logger.info(
            f"Fitting preprocessor on {X.shape[0]} normal samples "
        ...
        if self.apply_pca:
            self.pca.fit_transform(X_scaled)
    ...
    def load(cls, path: str) -> "UnsupervisedPreprocessor":
        ...
        preprocessor.explained_variance_ratio_ = preprocessor_data.get(
    ...
    def __repr__(self) -> str:
        ...
        if self.apply_pca:
            if self.explained_variance_ratio_ is not None:
                ...
                return (
                    f"UnsupervisedPreprocessor(n_components={self.n_components}, "
                    f"variance_explained={variance:.2%}, {status})"
                )
            return (
                f"UnsupervisedPreprocessor(n_components={self.n_components}, {status})"
            )
