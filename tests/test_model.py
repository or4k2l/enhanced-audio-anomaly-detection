...
@pytest.mark.skipif(not HAS_TF, reason="TensorFlow nicht installiert")
class TestAutoencoderAnomalyDetector:
    """Test suite for AutoencoderAnomalyDetector."""


    def test_initialization(self):
        detector = AutoencoderAnomalyDetector(encoding_dim=8, random_state=42)


class TestXGBoostAnomalyDetector:
    """Test suite for XGBoostAnomalyDetector."""
    ...
    """Test suite for RandomForestAnomalyDetector."""
    ...
    def test_evaluate(self):
        ...
        X_test = np.random.randn(50, 20)
        y_test = np.random.randint(0, 2, 50)
        # Dummy evaluation: just check predict shape and value range
        pred = detector.predict(X_test)
        assert pred.shape[0] == 50
        assert set(pred).issubset({0, 1})
